...
func (wg WireGrid) returnCollisions() []Point {
	points := []Point{}
	for point, grid := range wg {
		if grid.hit == 2 {
...
func (wg WireGrid) returnStepsForCollisions() []int {
	stepsTaken := []int{}
	for _, grid := range wg {
		if grid.hit >= 2 {
			stepsTaken = append(stepsTaken, grid.steps)
		}
	}
	// fmt.Println("stepsTaken", stepsTaken)
	return stepsTaken
}

func (wg WireGrid) calculateClosestCollisionDistance() int {
	collisions := wg.returnCollisions()
	lowestDistance := 0
	for _, collision := range collisions {
		sum := int(math.Abs(float64(collision.x)) + math.Abs(float64(collision.y)))
		if lowestDistance == 0 {
			lowestDistance = sum
		}
		if lowestDistance != 0 && sum < lowestDistance {
			lowestDistance = sum
		}
	}
...
func (wg WireGrid) calculateFewestStepsForCollision() int {
	steps := wg.returnStepsForCollisions()
	lowestSteps := 0
	for _, step := range steps {
		if lowestSteps == 0 {
			lowestSteps = step
		}
		if lowestSteps != 0 && step < lowestSteps {
			lowestSteps = step
		}
	}
	return lowestSteps
	...
	for _, instruction := range instructionsSplit {
		...
		if direction == "R" {
			for i := 1; i <= distance; i++ {
				...
				wg.plotGrid(newPoint, newDistance, steps)
...
func (wg WireGrid) plotGrid(newPoint Point, newDistance int, steps int) {
	if _, ok := wg[newPoint]; ok {
		...
		gridBlock := wg[newPoint]
		if gridBlock.hit == 1 {
			gridBlock.hit++
			gridBlock.steps += steps
		}
	} else {
