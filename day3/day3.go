...
func NewWireGrid(instructions string) WireGrid {
	wg := make(WireGrid)
	wg.calculateAndAddPoints(instructions)
	return wg
}
...
func returnCollisions(wg1, wg2 WireGrid) []Point {
	collisions := []Point{}
	for point := range wg2 {
		if _, ok := wg1[point]; ok {
			collisions = append(collisions, point)
		}
	}
	return collisions
}

func returnCollisionsSteps(wg1, wg2 WireGrid) int {
	steps := []int{}
	for point := range wg2 {
		if _, ok := wg1[point]; ok {
...
func main() {
	...
	wireGrid1 := NewWireGrid(wire1)
	wireGrid2 := NewWireGrid(wire2)
	fmt.Println(returnCollisionsSteps(wireGrid1, wireGrid2))
}
